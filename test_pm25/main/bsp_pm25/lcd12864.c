...
extern void IIC_Init(void); //IIC接口实现
...
void OLED_ShowFloat(u8 x, u8 y, float num, u8 len, u8 size)
{
	...
	if (len)
		OLED_ShowChar(x + (l_float + 1) * 8, y, '.', 8);
...
void Picture()
{
	...
	{
		OLED_WR_Byte(0xb0 + y, OLED_CMD);
		OLED_WR_Byte(0x0, OLED_CMD);
		OLED_WR_Byte(0x10, OLED_CMD);
		for (x = 0; x < 132; x++)
		{
			OLED_WR_Byte(BMP2[i++], OLED_DATA);
		}
	}
}

/***********功能描述：显示显示BMP图片128×64起始点坐标(x,y),x的范围0～127，y为页的范围0～7*****************/
void OLED_DrawBMP(unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1, const unsigned char *arg)
{
	unsigned int j = 0;
	unsigned char x, y;
