...
/* 单片机的引脚    CH376芯片的引脚
      ...
      P1.7                 SCK      */
static u8 ft = 0;
#define	P14		ft=3;while(ft--);PFout(1)
#define	P15		ft=3;while(ft--);PFout(3)
#define	P16		PFin(5)
#define	P17		ft=3;while(ft--);PFout(7)
...
#define CH376_INT_WIRE			PFin(9)	/* 假定CH376的INT#引脚,如果未连接那么也可以通过查询兼做中断输出的SDO引脚状态实现 */
...
/* 如果是硬件SPI接口,那么可使用mode3(CPOL=1&CPHA=1)或mode0(CPOL=0&CPHA=0),CH376在时钟上升沿采样输入,下降沿输出,数据位是高位在前 */
  ...
  GPIO_Init(GPIOF, &GPIO_InitStructure);
	...
	CH376_SPI_SCK = 1;  /* 默认为高电平,SPI模式3,也可以用SPI模式0,但模拟程序可能需稍做修改 */
...
void	xEndCH376Cmd( )	{ CH376_SPI_SCS = 1; }  /* SPI片选无效,结束CH376命令,仅用于SPI接口方式 */
