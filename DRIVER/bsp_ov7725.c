...
//	.lcd_sx = 0,
//	.lcd_sy = 0,
//	.lcd_scan = 1, //LCD扫描模式，本横屏配置可用1、3、5、7模式
//	
...
static void VSYNC_GPIO_Config(void)
{
	...
	SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOG, EXTI_PinSource7);//PA0 连接到中断线0
...
volatile u8 picture[4800];
void ImagDisp()
{
//	u16 i,j;
//	//LCD_OpenWin(0,0,320,240 );
//        u18 p=0;
//        for(i=0;i<240;i++)
//          for(j=0;j<320;j++)
//        {
...
//                        if(j%4==0&&i%4==0){if(Camera_Data<20000){picture[p++]=0;}//if(j==5)usart1_send_char(0),j=0;}
//                        else {picture[p++]=1;}//if(j==5)usart1_send_char(0X11),j=0;}
//			Driver_LcdFSMCWriteData(Camera_Data);//}
//                        
//		
//	}
//        //for(i=0;i<4800;i+=2)usart1_send_char(picture[i]);
//        //delay_ms(1000);
//        for(i=0;i<80;i++)
//          for(j=0;j<60;j++)
//            OLED_DrawPoint(i,j,picture[j+i*60]);
//        OLED_Refresh_Gram();
        //for(i=0;i<4800;i+=2)
        //usart1_send_char(picture[i]);
        //delay_ms(1000);
//	LCD_Scan_Dir(DFT_SCAN_DIR);	//恢复默认扫描方向 
  
        u16 i,j,p;
        p=0;
        LCD_OpenWin(0,0,119,159 );
        for(i=0;i<240;i++)
          for(j=0;j<320;j++)
        {
			...
			READ_FIFO_PIXEL(Camera_Data);
                        if(j%2==0&&i%2==0)
                        {
                          ...
                          Driver_LcdFSMCWriteData(Camera_Data);
                          //OLED_DrawPoint(j/4,i/4,Camera_Data);
			}
